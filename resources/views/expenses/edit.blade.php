    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-600">Modifier les informations de la charge</p>
            </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required 
                            ...
                            <option value="rent" {{ old('type', $expense->type) === 'rent' ? 'selected' : '' }}>Loyer</option>
                            <option value="electricity" {{ old('type', $expense->type) === 'electricity' ? 'selected' : '' }}>Électricité</option>
                            <option value="agent_expense" {{ old('type', $expense->type) === 'agent_expense' ? 'selected' : '' }}>Dépense Agent</option>
                            <option value="general" {{ old('type', $expense->type) === 'general' ? 'selected' : '' }}>Autre / Général</option>
                        </select>
                        @error('type')
                    ...
                    <div id="agentField" style="display: {{ old('type', $expense->type) === 'agent_expense' ? 'block' : 'none' }};">
                        ...
                        <select name="user_id" id="user_id" 
                            ...
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}" {{ old('user_id', $expense->user_id) == $agent->id ? 'selected' : '' }}>
                                    {{ $agent->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" required
                                  ...
                                  placeholder="Décrivez la nature de cette charge...">{{ old('description', $expense->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date" required
                               value="{{ old('date', $expense->expense_date->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="2"
                                  ...
                                  placeholder="Informations supplémentaires...">{{ old('notes', $expense->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
