...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl shadow-lg p-6 border border-blue-200">
        ...
        <div style="height: 320px; position: relative;">
            <div id="paymentsChartLoader" class="flex items-center justify-center h-full">
                <div class="text-center">
                    <svg class="animate-spin h-8 w-8 text-blue-600 mx-auto mb-2" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <p class="text-blue-600 text-sm">Chargement...</p>
                </div>
            </div>
    ...
    <div class="bg-gradient-to-br from-yellow-50 to-orange-100 rounded-xl shadow-lg p-6 border border-yellow-200">
        ...
        <div style="height: 320px; position: relative;">
            <div id="amountsChartLoader" class="flex items-center justify-center h-full">
                <div class="text-center">
                    <svg class="animate-spin h-8 w-8 text-yellow-600 mx-auto mb-2" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <p class="text-yellow-600 text-sm">Chargement...</p>
                </div>
            </div>
            <canvas id="amountsChart" style="display: none;"></canvas>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    ...
    <div class="bg-gradient-to-br from-purple-50 to-violet-100 rounded-xl shadow-lg p-6 border border-purple-200">
        ...
        <div style="height: 320px; position: relative;">
            <div id="agentsChartLoader" class="flex items-center justify-center h-full">
                <div class="text-center">
                    <svg class="animate-spin h-8 w-8 text-purple-600 mx-auto mb-2" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    <p class="text-purple-600 text-sm">Chargement...</p>
                </div>
            </div>
            <canvas id="agentsChart" style="display: none;"></canvas>
        </div>
    </div>
</div>
...
async function loadChartsData() {
    console.log('Chargement asynchrone des données des graphiques...');
    
    try {
        const response = await fetch('{{ route("api.dashboard.charts") }}', {
            method: 'GET',
            headers: {
        ...
        // Initialiser les graphiques avec les données reçues
        initializeCharts(data);
        
    } catch (error) {
        console.error('Erreur lors du chargement des données:', error);
        // Afficher un message d'erreur dans les loaders
        document.querySelectorAll('[id$="Loader"]').forEach(loader => {
            loader.innerHTML = '<div class="text-center"><p class="text-red-600 text-sm">Erreur de chargement</p></div>';
        });
    }
}
...
function initializeCharts(data) {
    ...
    console.log('Chart.js disponible, initialisation des graphiques...');
    
    const paymentsData = data.payments_chart_data;
    const amountsData = data.amounts_chart_data;
    const agentsData = data.agents_performance;
    ...
    if (ctx1 && paymentsData) {
        ...
        hideLoader('paymentsChart');
        console.log('Graphique paiements créé avec', paymentsData.data.length, 'points de données');
    }
    ...
    if (ctx4 && agentsData && agentsData.length > 0) {
        ...
        hideLoader('agentsChart');
        console.log('Graphique agents créé avec', agentsData.length, 'agents');
    }
}

// Démarrer le chargement des données dès que la page est prête
document.addEventListener('DOMContentLoaded', function() {
    // Attendre un peu que Chart.js soit chargé
