...
class TontineController extends Controller
{
     ...
     */
    public function markAsDelivered(Tontine $tontine)
    {
        // Vérifier que la tontine est terminée (tous les paiements validés)
        if ($tontine->status !== 'completed') {
            return back()->withErrors(['error' => 'Cette tontine n\'est pas encore terminée. Tous les paiements doivent être validés avant la livraison.']);
        }

        // Vérifier que la tontine n'est pas déjà livrée
        if ($tontine->delivery_status === 'delivered') {
            return back()->withErrors(['error' => 'Cette tontine a déjà été marquée comme livrée.']);
        }
        ...
        try {
            ...
            $tontine->update([
                ...
                'delivery_notes' => request('delivery_notes', 'Livraison confirmée via l\'interface')
            ]);
            ...
            if ($product && $product->stock_quantity > 0) {
                ...
                // Log de l'opération
                \Log::info('Déstockage automatique lors de la livraison', [
                    'tontine_id' => $tontine->id,
                    'product_id' => $product->id,
                    'product_name' => $product->name,
                    'client' => $tontine->client->full_name,
                    'ancien_stock' => $product->stock_quantity + 1,
                    'nouveau_stock' => $product->stock_quantity,
                    'livre_par' => auth()->user()->name,
                    'date_livraison' => now()
                ]);
            }
            ...
            return back()->with('success', 'Tontine marquée comme livrée ! Le stock a été automatiquement décrémenté.');

        } catch (\Exception $e) {
            DB::rollBack();
            return back()->withErrors(['error' => 'Erreur lors de la livraison : ' . $e->getMessage()]);
        }
    }
}
